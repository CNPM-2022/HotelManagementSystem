                            ...
                            <Fragment key={room._id}>
                                <div className="single-room-area d-flex align-items-center mb-5 justify-content-center "
                                    ...
                                    <div className="room-content">
                                        ...
                                        <h4>{room.price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")} VNĐ <span>/ Ngày</span></h4>
                                        <div className="room-feature">
                                            <h6>Loại: <span>{room.type}</span></h6>
                                            <h6>Sức chứa: <span>Max persion {room.maxCount}</span></h6>
                                            <h6>Dịch vụ: <span>{room.note}</span></h6>
                                        </div>
            ...
            >
                <ul className="pagination pagination-allroom" ref={pageRef}>
                    <li className="page-item" id="pre-btn">
                        <div className="page-link" onClick={handlePrePage} ><i className="fa fa-angle-left"></i>Trước </div>
                    </li>
                    {
                        listPage.map(page => (
                            <li className="page-item page" key={page}><NavLink to={`/search/${page}`} className="page-link" onClick={handleChangePage}>{page}</NavLink></li>
                        ))
                    }
                    <li className="page-item" id="next-btn">
                        <div className="page-link" onClick={handleNextPage}>Sau <i className="fa fa-angle-right"></i></div>
                    </li>
                </ul>
            </nav>
        </div>
