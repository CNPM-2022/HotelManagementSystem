    ...
    const onStepSubmit = useCallback(event => {
        ...
        if (step === 0) {
            ...
            for (let i = 1; i <= values.amountCustomer; i++) {
                ...
                tempUser.typeUser = _.at(_.pick(values, [i.toString(), 'type' + i.toString()]), 'type' + i.toString())[0];
        ...
        if (step === 1) {
            ...
            if (room.price <= 1000000 && room.price >= 500000) { price = '3' }
            const data = {
                "type": room.type,
                "price": price,
        ...
        <div className='h-100 booking-container' style={{ backgroundColor: "rgb(238,238,238)" }}>
            <div className='form-booking-container h-100 my-5'>
                <div className='  ' style={{
                    ...
                    <Form initialValues={formState} onSubmitClick={onStepSubmit} render={formRenderProps => <div className="d-flex justify-content-center booking-form">
            ...
            <div className='infor-room'>
                {room._id && <div className="card" style={{ width: 18 + "rem" }}>
                    <img src={`http://localhost:5000/${room.imageUrls[0].filePath}`} width={150} height={150} className="card-img-top" alt="..." />
                    <div className="card-body">
                        <h5 className="card-title border-bottom">Phòng #{room.roomNumber}</h5>
                        <div className="room-price"><b>Giá:  </b> {room.price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")} VNĐ <span>/ Ngày</span></div>
                        <div><b>Loại: </b><span>{room.type}</span></div>
                        <div><b>Số lượng: </b><span>Tối đa {room.maxCount} người</span></div>
                        <div><b>Trạng thái: </b><span>King beds</span></div>
                        <div><b>Dịch vụ: </b><span>{room.note}</span></div>
